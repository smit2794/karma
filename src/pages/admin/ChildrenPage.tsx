...
export default function ChildrenPage() {
  ...
  const renderChildDetail = (child: Child) => {
    ...
    return (
      <div className="flex flex-col h-[92vh] overflow-hidden rounded-3xl" style={{ background: '#F4F8FF' }}>
        {/* ── HERO HEADER ── */}
        <div
          className="shrink-0 relative overflow-hidden"
          style={{ background: 'linear-gradient(135deg, #091E3A 0%, #0B6CC4 45%, #5DBCEB 75%, #F28C28 100%)', minHeight: 160 }}
        >
          {/* decorative rings */}
          ...
          <div className="relative z-10 p-6 flex items-start gap-5">
            ...
            <div className="relative shrink-0">
              ...
              <span className="absolute bottom-1 right-1 h-4 w-4 bg-emerald-400 border-2 border-white rounded-full animate-pulse" />
            </div>
            ...
            <div className="hidden md:flex flex-col items-end gap-2 text-right">
              <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl px-4 py-2">
                ...
                <p className="text-[10px] text-white/70 font-medium">Visits</p>
              </div>
              <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl px-4 py-2">
                <p className="text-2xl font-black text-white">{child.interventions.length}</p>
                <p className="text-[10px] text-white/70 font-medium">Interventions</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col p-6 overflow-hidden">
          <Tabs defaultValue="overview" className="flex-1 flex flex-col overflow-hidden w-full">
            <TabsList
              className="w-full justify-start bg-slate-900/5 backdrop-blur-md p-1 h-auto space-x-1 border border-slate-200/50 rounded-2xl mb-4"
            >
              {[
                ...
                <TabsTrigger
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold data-[state=active]:bg-white data-[state=active]:text-[#0B6CC4] data-[state=active]:shadow-sm data-[state=inactive]:text-slate-600 transition-all hover:bg-white/40"
                >
            ...
            {/* ── Scrollable body ── */}
            <div
              className="overflow-y-auto flex-1 rounded-2xl border border-slate-200/80 bg-white"
